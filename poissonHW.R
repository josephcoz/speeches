# read in raw data
speechesraw <- read.table(text = " wcnt year    budget inflate thanksP  man   woman  thanksM  thanksW  time
  212 1942   1344000   16.06  3       101     452        1        2   108
                       ...
                       213 2017  19400000    1.04  4       293     264        8        3   233", header = TRUE)

# extract data into three different dataframes
picture <- speechesraw[,c(2,1,5)]
man <- speechesraw[,c(2,6,8)]
woman <- speechesraw[,c(2,7,9)]

# rename columns to be the same
colnames(picture) <- c('year', 'words', 'thanks')
colnames(man) <- c('year', 'words', 'thanks')
colnames(woman) <- c('year', 'words', 'thanks')

# add type for award
picture$type <- rep('picture', dim(picture)[2])
man$type <- rep('man', dim(man)[2])
woman$type <- rep('woman', dim(woman)[2])

# put all together
speeches <- rbind(picture, man, woman)

# order by year
speeches <- speeches[order(speeches$year),]

# ANALYSIS
# Fit poisson model
# thanks = beta0 + beta1 * year + beta2 * words + beta3 * type
speeches_out <- glm(thanks ~ year + words + type, data = speeches, family = "poisson")
summary(speeches_out)

# To better understand the effects
exp(coef(speeches_out)[-1])

# 95% CI
exp(confint(speeches_out)[-1,])

# Does award type have effect on number of 'thanks'?
speeches_red <- glm(thanks ~ type, data = speeches, family = "poisson")
anova(speeches_red, test = "Chisq")
